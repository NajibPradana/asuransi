                                            ...
                                            <div class="flex items-center justify-between pt-4 mt-auto border-t border-background-light">
                                                <div class="flex items-center">
                                                    @if($article->author && $article->author->profile_photo_path)
                                                        <img src="{{ Storage::url($article->author->profile_photo_path) }}" alt="{{ $article->author->fullname }}" class="mr-3 border-2 border-white shadow-sm w-9 h-9">
                                                    @else
                                                        <img src="https://placehold.co/40x40?text={{ substr($article->author->fullname ?? 'A', 0, 2) }}" alt="{{ $article->author->fullname ?? 'Author' }}" class="mr-3 border-2 border-white shadow-sm w-9 h-9">
                                                    @endif
                                                    <span class="text-sm font-medium text-primary-700">{{ $article->author->fullname ?? 'Anonymous' }}</span>
                                                </div>
